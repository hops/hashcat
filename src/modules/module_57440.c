...
static const char *HASH_NAME      = "Western Digital JMS538S KEK";
static const u64   KERN_TYPE      = 57440;
...
static const char *ST_HASH        = "73656c667465737464697361626c6564";
...
static const int   ROUNDS_WD_1K         = 1000;
...
{
  const u64 esalt_size = (const u64) sizeof (wd_t);
...
{
  const u64 tmp_size = (const u64) sizeof (wd_tmp_t);
...
{
  // Currently limited to 28 characters because there's something in the KDF
  // I haven't fully wrapped my head around yet. It's not straight forward
  // if the length after UTF-16LE expansion is greater than 64 bytes.
  // Since the salt takes up 4 characters the max pw length is set to 28
  // 4 (salt) + 28 (password) * 2 (UTF-16LE)
  const u32 pw_max = 28;
...
int module_hash_binary_parse (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED const user_options_t *user_options, MAYBE_UNUSED const user_options_extra_t *user_options_extra, hashes_t *hashes)
{
  ...
  #define WD_HEADER_SIZE 512

  char *in = (char *) hcmalloc (WD_HEADER_SIZE);

  const size_t n = hc_fread (in, 1, WD_HEADER_SIZE, &fp);

  hc_fclose (&fp);

  if (n != WD_HEADER_SIZE) return (PARSER_WD_FILE_SIZE);

  if (memcmp (in, "WDv1", 4)) return (PARSER_WD_MAGIC);

  hash_t *hashes_buf = hashes->hashes_buf;

  hash_t *hash = &hashes_buf[0];

  const int parser_status = module_hash_decode (hashconfig, hash->digest, hash->salt, hash->esalt, hash->hook_salt, hash->hash_info, in, WD_HEADER_SIZE);
...
int module_hash_decode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED void *digest_buf, MAYBE_UNUSED salt_t *salt, MAYBE_UNUSED void *esalt_buf, MAYBE_UNUSED void *hook_salt_buf, MAYBE_UNUSED hashinfo_t *hash_info, const char *line_buf, MAYBE_UNUSED const int line_len)
{
  ...
  wd_t *wd = (wd_t *) esalt_buf;

  // skip the first 6 blocks
  u32 *enc_keyblock_ptr = (u32 *) (line_buf + 96);

  // reverse each 16 byte block
  for (int i = 0; i < 104; i += 4)
  {
      wd->data_buf[i + 0] = enc_keyblock_ptr[i + 3];
      wd->data_buf[i + 1] = enc_keyblock_ptr[i + 2];
      wd->data_buf[i + 2] = enc_keyblock_ptr[i + 1];
      wd->data_buf[i + 3] = enc_keyblock_ptr[i + 0];
  }

  salt->salt_iter = ROUNDS_WD_1K - 1;

  digest[0] = wd->data_buf[0];
